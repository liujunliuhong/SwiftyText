//
//  BaseTextUnderlineStyleViewController.swift
//  SwiftyText
//
...
class BaseTextUnderlineStyleViewController: BasePageViewController {
...
extension BaseTextUnderlineStyleViewController {
    override func makeUI() {
        ...
        let u1 = SwiftyTextUnderLine()
        u1.firstLineStyle = SwiftyTextLineStyle.single(strokeWidth: 1, strokeColor: .cyan, lineJoin: .round, lineCap: .round)
        atr1.st_add(underline: u1)
        ...
        let u2 = SwiftyTextUnderLine()
        u2.firstLineStyle = SwiftyTextLineStyle.single(strokeWidth: 1, strokeColor: .cyan, lineJoin: .round, lineCap: .round) // 第一条线
        u2.secondLineStyle = SwiftyTextLineStyle.single(strokeWidth: 3, strokeColor: .purple, lineJoin: .round, lineCap: .round) // 第二条线
        u2.lineSpace = 2.0 // 两条线之间的间距
        atr2.st_add(underline: u2)
        ...
        // 使用系统的
        //atr3.yy_setUnderlineStyle(.double, range: NSRange(location: 0, length: atr2.length))
        //atr3.yy_setUnderlineColor(.cyan, range: NSRange(location: 0, length: atr2.length))
        
        // 使用YY封装的
        let u3 = YYTextDecoration(style: .double)
        u3.color = .cyan
        u3.width = 1.0
        atr3.yy_setTextUnderline(u3, range: NSRange(location: 0, length: atr2.length))
